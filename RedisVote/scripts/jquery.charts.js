	...
	$.fn.charts = function (settings) {
		...
		return this.each(function () {
			...
			if (config.chartbgcolours.length != config.chartfgcolours.length) {
				console.log("Invalid settings, chartfgcolours must be same length as chartbgcolours");
			}
			...
			switch (config.charttype) {
				...
				case 'waterfall':
					...
					switch (config.direction) {
						...
						case 'horizontal':
							// Horizontal Bars
							console.log("Horizontal waterfall charts not yet supported!");
							break;
			...
			*/
			...
			$("." + config.classmodifier + "bar").each( function() {
				...
				switch (config.direction) {
					case "horizontal":
						$(this).width(calculatedSize+"%");
						break;
					case "vertical":
						$(this).height(calculatedSize+"%");
						break;
				}
			});
